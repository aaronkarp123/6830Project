...
public class BTreeFile implements DbFile {
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// should be inserted.
            ...
            for (int j = 0; j < numEs / 2; j ++) {
                if (!es.hasNext()) throw new DbException("No more entries.");
                BTreeEntry e = es.next();
                page.deleteKeyAndLeftChild(e);
                npage.insertEntry(e);
            }
            // Pick the one in the middle
            BTreeEntry e_push  = es.next();
            Field f_push = e_push.getKey();
            page.deleteKeyAndLeftChild(e_push);
            e_push = new BTreeEntry(f_push, npage.getId(), page.getId());
            // Update all children affected
            updateParentPointers(tid, dirtypages, page);
            updateParentPointers(tid, dirtypages, npage);
            // Handle the parent
            BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), f_push);
            // Link the (probably new) parent and both npage and page
            parent.insertEntry(e_push);
            updateParentPointers(tid, dirtypages, parent);
            // Compare the input field with the new field in parent
            return (f_push.compare(Op.GREATER_THAN_OR_EQ, field)) ? npage : page;
	}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		// the corresponding parent entry.
            ...
            Iterator<Tuple> ts = (isRightSibling) ? sibling.iterator() : sibling.reverseIterator();
            if (ts == null || !ts.hasNext()) throw new DbException("Illegal tuple iterator.");
            Tuple t = null;
            for (int j = 0; j < numSteal; j ++) {
                if (!ts.hasNext()) throw new DbException("No more tuples.");
                t = ts.next();
                sibling.deleteTuple(t);
                page.insertTuple(t);
            }
            // t has the new key field that should be updated in their parent
            if (isRightSibling) t = ts.next();
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// deleteParentEntry() will be useful here
            Iterator<Tuple> ts = rightPage.iterator();
            if (ts == null || !ts.hasNext()) throw new DbException("Illegal tuple iterator.");
            while (ts.hasNext()) {
                Tuple t = ts.next();
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// deleteParentEntry() will be useful here
            ...
            Iterator<BTreeEntry> es = rightPage.iterator();
            if (es == null || !es.hasNext()) throw new DbException("Illegal tuple iterator.");
            while (es.hasNext()) {
                BTreeEntry e = es.next();
